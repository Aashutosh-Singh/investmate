...
def preprocess_data(df):
    ...
    try:
        ...
        # Force all numeric columns to float64 — modern pandas/yfinance may return
        # PyArrow-backed arrays which cause 'only integer scalar arrays' crash in sklearn
...
def label_risk(df):
    ...
    try:
        ...
        # NumPy 2.x requires default to have same dtype as choices (str, not int)
        df['Risk Level'] = np.select(conditions, choices, default='Medium')
        # Ensure plain Python strings — not PyArrow backed
...
def train_and_save_model(ticker, model_path, scaler_path):
    ...
    try:
        ...
        # Use to_numpy(dtype) to guarantee plain NumPy arrays regardless of PyArrow backing
        X = data[features].to_numpy(dtype='float64')
        y = data['Risk Level'].astype(str).to_numpy()

        # Check if we have enough samples per class for stratified splitting
        unique_classes, class_counts = np.unique(y, return_counts=True)
        min_class_count = class_counts.min()

        # Need at least 5 samples per class for cv=5 with stratify
        ...
        # Use a simpler, faster grid if data is small
        if len(X_train) < 100 or not use_stratify:
            # Fallback: simple RandomForest, no grid search
...
def risk_analysis_model(new_stock_ticker):
    ...
    # Scale the latest data — force plain numpy array to bypass PyArrow
    latest_data_scaled = scaler.transform(latest_data.to_numpy(dtype='float64').reshape(1, -1))
...
def fetch_risk_results(new_stock_ticker, portfolio):
    ...
    try:
        # Only train if no model exists yet — use cache on subsequent calls
        if not os.path.exists(model_path) or not os.path.exists(scaler_path):
            model, scaler = train_and_save_model(new_stock_ticker, model_path, scaler_path)
            print(f"Model trained for {new_stock_ticker}...")
        else:
            print(f"Using cached model for {new_stock_ticker}...")
